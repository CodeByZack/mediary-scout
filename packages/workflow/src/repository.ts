...
import type { PromptOverride, RulePattern } from "./ruleset.js";
import type {
  Account,
  ConnectedStorage,
  Session,
 ...
 * drive's episodes clobber or shadow another's. The unit separator (0x1f) cannot
 * appear in ids - and unlike NUL it keeps this file diffable as text, so git stops
 * treating the whole 60 KB file as binary. A null/undefined storage collapses to
 * the sentinel so a key always exists.
 */
...
export interface WorkflowRepository extends DeadLinkStore {
  ...
  /** 集数解析规则配置（issue #44）：全部规则行，按 sort_order 升序。 */
  listRulePatterns(): Promise<RulePattern[]>;
  /** 整体替换集数解析规则（保存 / 恢复默认都走这里；空数组 = 清空 → 回退内置）。 */
  replaceRulePatterns(patterns: RulePattern[]): Promise<void>;
  /** AI 仲裁 prompt 覆盖（issue #44）：全部覆盖行。 */
  listPromptOverrides(): Promise<PromptOverride[]>;
  /** 整体替换 prompt 覆盖（空数组 = 清空 → 用内置 prompt）。 */
  replacePromptOverrides(overrides: PromptOverride[]): Promise<void>;
...
export class InMemoryWorkflowRepository implements WorkflowRepository {
  ...
  private readonly rulePatterns = new Map<string, RulePattern>();
  ...
  async listRulePatterns(): Promise<RulePattern[]> {
    return [...this.rulePatterns.values()].sort((a, b) => a.sortOrder - b.sortOrder);
  }

  async replaceRulePatterns(patterns: RulePattern[]): Promise<void> {
    this.rulePatterns.clear();
    for (const p of patterns) this.rulePatterns.set(p.ruleId, { ...p });
  }

  async listPromptOverrides(): Promise<PromptOverride[]> {
    return [...this.promptOverrides.values()];
  }

  async replacePromptOverrides(overrides: PromptOverride[]): Promise<void> {
    this.promptOverrides.clear();
    for (const o of overrides) this.promptOverrides.set(o.arbitrationKind, { ...o });
  }
  ...
  async upsertConnectedStorage(row: UpsertConnectedStorageInput): Promise<void> {
    ...
    this.connectedStorages.set(key, {
      ...
      varietyCid: row.varietyCid ?? null,
