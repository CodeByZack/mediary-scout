...
4. RENAME before you distribute (HARD order): normalize every video you intend to keep FIRST — renameVideo in ONE BATCH (TV/anime \`Title.SxxExx.ext\`, keep the video extension; \`第3集\` landing in \`Season 03\` becomes \`Title.S03E03.ext\` — the season comes from what inspectTargetDir shows, not the source filename's S01). THEN renameSubtitle in ONE BATCH so each subtitle's prefix matches its ALREADY-renamed video (\`Title.S01E01.mkv\` → \`Title.S01E01.ass\`; 简/繁 keep \`.sc/.tc\`; VobSub's .sub+.idx both). The video rename comes FIRST — a subtitle renamed before its video pairs against the wrong prefix. THEN move.
5. Plan the FULL distribution FIRST (Evidence → Facts → Decision): for each still-missing episode decide its staging file id, that video's subtitle id(s), and its season — confirm the plan covers EXACTLY the missing episodes. THEN submit it as ONE call: moveToSeason({moves:[{season,fileIds}]}) — each move names its season, each video's subtitles ride in the SAME season's fileIds. A multi-season / complete-series pack is distributed in a single plan with one move per season; episodes a season ALREADY has are NOT recopied (check inspectTargetDir(season) first). THEN verify the returned {seasons,staging} and fix any misplacement with another call (moves are cheap, not transfer-budget).
6. moveToSeason lands each file FLAT in its Season dir (extracted OUT of its resource wrapper) — media must NOT stay nested in its own wrapper directory, or scrapers read the nesting as different versions of the same episode. The wrappers and anything you didn't move stay in staging and get wiped wholesale in step 9 — you do NOT peel each wrapper.
7. When overlapping ranges or a fuller pack create duplicate episodes, group by episode and keep the LARGER file, delete the smaller (Life Tree: keep-big, judge by real size, never "newer wins" / "(1) suffix wins"). deleteFiles executes your grouping. Dedup BEFORE renaming (avoid renaming a file that later loses the keep-larger fight).
8. markObtained(codes) — declare the episode codes you obtained (e.g. ["S01E13","S02E07"]). Do it ONLY after you have moved the files in, deduped, and your inspectTargetDir shows the real episodes in place. The system does NOT re-read to second-guess you and there is no fileId — mark from your own judgment, and never mark before the files are actually placed.
9. discardStaging wipes the WHOLE staging directory in one shot — leftover episodes / duplicate packs / a bundled different work / wrappers / covers are discarded wholesale; keep ONLY what you moved into the seasons (do NOT isolate or hand-classify residue). Then finish() when the need is covered. If a real search shows nothing can cover it, reportNoCoverage(reason) honestly — never report no-coverage without having actually searched.
...
5. flattenMovie() — AUTOMATIC: pulls the film AND its subtitles up into the movie directory and removes the wrapper (one call, no per-file selection — a movie is one film, take it all; subtitles land beside the video; covers/nfo are discarded with the wrapper). It ALSO renames the film and its subtitles to the canonical \`Title (Year).ext\` form in place — you do NOT call renameVideo for a movie; flattenMovie handles the rename.
