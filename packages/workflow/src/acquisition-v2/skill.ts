...
1. \`renameVideo({ renames:[{fileId,newName},…] })\` — ONE BATCH call for ALL videos. TV/anime: \`Title.SxxExx.ext\` (视频扩展名不变;必须带集号;season 以目标季为准 — \`第3集\` 进 \`Season 03\` 就写 \`S03E03\`,不是源文件里的 S01)。
...
3. renameSubtitle({ renames: [{fileId, newName}, …] }) — ONE BATCH call for ALL landed subtitles: decide every subtitle↔episode pairing (fileIds from inspectStaging), then submit them together. **Call this AFTER \`renameVideo\`** — each newName = the matching video's canonical prefix + the subtitle extension (Title.S01E01.mkv → Title.S01E01.ass; 简/繁 double subs keep their .sc/.tc infix). Subtitles are renamed to match their videos (the scraper auto-loads them) — the video rename comes FIRST so the prefix is canonical. NEVER rename one file per call — a 77-episode season is one batch call, not 77 calls (the lived 2026-07-02 lesson: per-file calls collapse at scale and most episodes ship unpaired). Per-item guard violations return in the "errors" list; fix or drop those items, don't retry the whole batch.
