...
/** 隐形季号(issue #21):`奇异新世界3.全集` — 紧贴「全集/合集/完结」的季号。`(?<![\d.])`
 *  让年份与日期残段不中(`2019.全集` 的 19 前有数字、`2026.09.完结` 的 09 前有点);
 *  「全20集」没有数字+点+全集形态,不中。2026-09-12 补「完结」:「花儿与少年4.完结」的
 *  4 此前隐形 —— 完结不在季号正则里,却在 COMPLETE_MARKER 里被当成全集证据,于是第 4
 *  季包在 S8 任务下判 A 盲转 → 假入库。 */
const SEASON_MARKER_DOT = /(?<![\d.])(\d{1,2})\s*[.．]\s*(?:全集|合集|完结)/;
