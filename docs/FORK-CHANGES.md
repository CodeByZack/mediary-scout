...
- 六轮反馈(用户实测):「获取全剧」(type1 series)的通知子 run id 带 `_s{季号}` 尾缀(persistSeriesSeasons),agent_steps 却记在无尾缀主 run → UI 用尾缀 id 查步骤恒空「暂无步骤记录」。修:activity-view runSteps 对带 `_s{数字}` 尾缀的 runId 剥尾缀回退查主 run,测试钉 series 用例。
